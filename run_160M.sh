...
LOGFILE="logs/train_ae_160M_$(date +%Y%m%d_%H%M%S).log"
...
torchrun --nproc_per_node=1 --nnodes=1 -m llmonade.train \
  ...
  --job.dump_folder exp/autoencoder_160M \
  --model.config llmonade/configs/joyce/autoencoder.json \
