...
def extract_digit_lookup_parts(text):
    ...
    # Expected format: "key:value " (note the trailing space)
    if ":" in text:
        parts = text.split(":", 1)
        key = parts[0].strip()
        # Get the value and remove any trailing space
...
def evaluate_batch(model, tokenizer, batch_texts, device="cuda"):
    ...
    for text in batch_texts:
        ...
        # Since each digit is typically one token, use the string length
        # This avoids tokenization issues
        answer_length = len(true_answer)
        ...
        if question in generated:
            pred_answer = generated[len(question) :].rstrip()
        else:
            pred_answer = generated.rstrip()
...
def main():
    ...
    for test_type in ["test_known_pairs", "test_novel_pairs"]:
        ...
        # Load dataset - use parquet loader with explicit file path
