...
def calculate_active_parameters(config: Dict[str, Any]) -> Dict[str, int]:
    ...
    mem_share_values_blocks = config.get("mem_share_values_blocks", 1)
    # Backward compatibility
    if "mem_share_values" in config and "mem_share_values_blocks" not in config:
        mem_share_values_blocks = 1 if config["mem_share_values"] else 0
    ...
    results = {
        ...
        "config_summary": {
            ...
            "mem_heads": mem_heads,
            "mem_knn": mem_knn,
            "mem_share_values_blocks": mem_share_values_blocks,
        },
    }
...
def print_results(results: Dict[str, int]):
    ...
    print(f"Memory Share Values Blocks: {config['mem_share_values_blocks']}")
    print()
