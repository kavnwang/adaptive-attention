#!/usr/bin/env python3
"""
Generate synthetic sorting dataset.

This script creates a dataset with key:value pairs where:
- Keys are N-digit numbers where each digit is in range [0, 9]
- Values are computed by sorting the digits of the key
...
def determine_n_from_num_keys(num_keys: int) -> Tuple[int, int]:
    """
    Calculate the minimum N such that 10^N >= num_keys.

    Args:
        num_keys: Number of distinct keys needed

    Returns:
        Tuple of (N, vocab_size) where vocab_size = 10^N
    """
    n = 1
    while 10**n < num_keys:
        n += 1
...
def generate_all_keys(n: int) -> List[str]:
    """
    Generate all valid N-digit numbers where each digit is in range [0, 9].
    ...
    # Generate all combinations of digits [0, 1, ..., 9]
...
def compute_sorted_value(key: str) -> str:
    """
    Sort the digits of the key to get the value.

    Args:
        key: N-digit key string

    Returns:
        N-digit value string with digits sorted
    """
    # Sort the digits of the key
    sorted_digits = sorted(key)
    return "".join(sorted_digits)


def create_dataset(num_keys: int, n: int, seed: int) -> List[Tuple[str, str]]:
    """
    ...
    all_numbers = generate_all_keys(n)
    ...
    pairs = []
    for key in keys:
        value = compute_sorted_value(key)
...
def generate_test_sets(
    training_pairs: List[Tuple[str, str]], n: int, num_test_pairs: int, seed: int
...
def main():
    parser = argparse.ArgumentParser(description="Generate synthetic sorting dataset")
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./synthetic_sorting_data",
    ...
    n, vocab_size = determine_n_from_num_keys(args.num_keys)
    print(f"\nUsing N={n} (digits 0-9), vocabulary size: {vocab_size:,}")

    # Create base dataset
    print("\nCreating base dataset...")
    pairs = create_dataset(args.num_keys, n, args.seed)
    ...
    known_pairs_test, novel_pairs_test = generate_test_sets(
        pairs, n, args.num_test_pairs, args.seed
    )
    ...
    metadata = {
        ...
        "transformation_rule": "Value = sorted digits of Key",
        "test_sets": {
