...
def parallelize_bento(
    ...
    # Apply memory parallelization for memory models
    # Check if this is a memory model by looking for HashingMemory modules
    has_memory_modules = any(
        isinstance(module, HashingMemory) for module in model.modules()
    )
    if has_memory_modules:
        apply_memory_parallel(
            model,
            world_mesh,
            parallel_dims,
            job_config,
    ...
    if (
        ...
        # Determine the appropriate mesh dimensions based on what's available
        available_dims = (
            world_mesh.mesh_dim_names if hasattr(world_mesh, "mesh_dim_names") else []
        )

        # Try to construct the dp_mesh based on available dimensions
        if parallel_dims.dp_replicate_enabled:
            # Try to find dp_replicate and dp_shard_cp dimensions
            if "dp_replicate" in available_dims and "dp_shard_cp" in available_dims:
                dp_mesh_dim_names = ("dp_replicate", "dp_shard_cp")
            elif "dp_replicate" in available_dims:
                dp_mesh_dim_names = ("dp_replicate",)
            elif "dp_shard_cp" in available_dims:
                dp_mesh_dim_names = ("dp_shard_cp",)
            else:
                # Fallback to the entire mesh if specific dimensions aren't found
                dp_mesh = world_mesh
                dp_mesh_dim_names = None
        else:
            # Try to find dp_shard_cp dimension
            if "dp_shard_cp" in available_dims:
                dp_mesh_dim_names = ("dp_shard_cp",)
            elif "dp" in available_dims:
        ...
        # Get the dp_mesh by slicing if we have specific dimensions
        if dp_mesh_dim_names is not None:
            try:
                dp_mesh = world_mesh[tuple(dp_mesh_dim_names)]
            except (KeyError, IndexError) as e:
                logger.warning(
                    f"Failed to slice mesh with dimensions {dp_mesh_dim_names}: {e}"
                )
                logger.info("Falling back to using the entire world mesh for FSDP")
                dp_mesh = world_mesh
...
def apply_memory_parallel(
    model: nn.Module,
    world_mesh: DeviceMesh,
    parallel_dims: ParallelDims,
    job_config: JobConfig,
):
    """Apply memory parallelization to all HashingMemory modules in the model."""

    # Use tensor parallel degree as memory parallel size if TP is enabled
    if parallel_dims.tp_enabled:
        memory_parallel_size = parallel_dims.tp
    else:
        memory_parallel_size = 1

    # Get param_dtype from job_config
    param_dtype = TORCH_DTYPE_MAP[job_config.training.mixed_precision_param]

    # Convert ALL parameters in the model to the target dtype to avoid FSDP mixed dtype issues
    # This matches the approach in the original implementation
    logger.info(
        f"Converting all model parameters to {param_dtype} to avoid FSDP mixed dtype issues"
    )
    with torch.no_grad():
    ...
    # Create simple wrapper objects to match original API signature
    class SimpleDistributedArgs:
        def __init__(self, memory_parallel_size):
            self.memory_parallel_size = memory_parallel_size

    class SimpleModelArgs:
        pass

    # Create the wrapper objects
    distributed_args = SimpleDistributedArgs(memory_parallel_size)
    model_args = SimpleModelArgs()

    # Apply memory parallelization to HashingMemory modules
    # Always call mp_parallelize - it handles the conditional logic internally
    for name, module in model.named_modules():
        if isinstance(module, HashingMemory):
            try:
                module.mp_parallelize(
                    mesh=world_mesh,
                ...
                logger.info(
                    f"Applied memory parallelization to {name} (memory_parallel_size={memory_parallel_size})"
                )
            except Exception as e:
                logger.warning(f"Failed to apply memory parallelization to {name}: {e}")
                logger.info("Continuing without memory parallelization for this module")
...
class MemoryTPPlan(TPPlan):
    """TP plan for memory models with HashingMemory modules."""

    def __init__(self, model=None, loss_parallel=False, enable_float8=False):
        super().__init__(model, loss_parallel, enable_float8)
        # Get memory layer indices from model config
        self.memory_layers = getattr(model.config, "memory_layers", []) if model else []
    ...
    @property
    def mlp_plan(self):
        # Return standard MLP parallelization plan
        # The apply_tp function will skip this for memory layers since they don't match the pattern
        return {
            "mlp": self.prepare_module_input(
                input_layouts=(Shard(1),),
                desired_input_layouts=(Replicate(),),
...
class PhiTransformerTPPlan(TransformerTPPlan):
    """TP plan for PhiTransformer model with PHi layer."""
    
    @property
    def model_plan(self):
        # Get base model plan from TransformerTPPlan
        plan = super().model_plan
        
        # Add PHi layer parallelization if it exists
        real_model = get_model(self.model)
        if hasattr(real_model, 'phi_layer') and real_model.phi_layer is not None:
            phi_layer_prefix = f"{self.base_model_prefix}.phi_layer"
            plan.update({
                # PHi layer linear projections
                f"{phi_layer_prefix}.posterior_mlp": self.colwise_parallel(),
                f"{phi_layer_prefix}.decoder_mlp": self.rowwise_parallel(output_layouts=Shard(1)),
                
                # PHi layer attention components
                f"{phi_layer_prefix}.prior_prediction_attention": self.prepare_module_input(
                    input_kwarg_layouts={"hidden_states": Replicate()},
                    desired_input_kwarg_layouts={"hidden_states": Replicate()},
                ),
                f"{phi_layer_prefix}.prior_prediction_attention.q_proj": self.colwise_parallel(),
                f"{phi_layer_prefix}.prior_prediction_attention.k_proj": self.colwise_parallel(),
                f"{phi_layer_prefix}.prior_prediction_attention.v_proj": self.colwise_parallel(),
                f"{phi_layer_prefix}.prior_prediction_attention.o_proj": self.rowwise_parallel(output_layouts=Replicate()),
                
                # PHi MLP components in prior_prediction_mlp
                f"{phi_layer_prefix}.prior_prediction_mlp.gate_layers": self.colwise_parallel(),
                f"{phi_layer_prefix}.prior_prediction_mlp.projection_layers": self.colwise_parallel(),
                f"{phi_layer_prefix}.prior_prediction_mlp.output_layer": self.rowwise_parallel(output_layouts=Replicate()),
                
                # Norm layers
                f"{phi_layer_prefix}.sa_norm": SequenceParallel(),
            })
...
TP_PLAN_MAP = {
    ...
    "gla": GLATPPlan,
    "memory": MemoryTPPlan,
    "memory_mlp_router": MemoryTPPlan,  # Memory MLP Router uses same TP plan as Memory
    "mlp_mlp_router": TransformerTPPlan,  # MLP MLP Router uses same TP plan as Transformer
    "phi_transformer": PhiTransformerTPPlan,  # PhiTransformer uses extended TransformerTPPlan
}
...
def apply_tp(
    ...
    if blocks is None:
        ...
        for layer_idx, block in enumerate(blocks):
            # Check if this block has a HashingMemory module
            has_memory = any(isinstance(m, HashingMemory) for m in block.modules())

            if has_memory:
                # For memory blocks, apply attention and norm parallelization
                # Memory modules are handled separately in apply_memory_parallel
                layer_plan = {
                    "attn_norm": SequenceParallel(),
                    **tp_plan.attn_plan,
                    "mlp_norm": SequenceParallel(),
                ...
                logger.debug(
                    f"Applying TP to memory block {layer_idx} (attention + norms)"
                )
            else:
                # For non-memory blocks, apply full parallelization
                layer_plan = tp_plan.layer_plan
                logger.debug(f"Applying TP to regular block {layer_idx} (full)")
...
def apply_fsdp(
    ...
    if blocks is None:
        ...
        for layer_id, block in enumerate(blocks):
            # Check if this block contains a HashingMemory module
            has_memory = any(isinstance(m, HashingMemory) for m in block.modules())

            if has_memory:
                # Skip FSDP for memory blocks since their values are already wrapped by mp_parallelize
                logger.debug(
                    f"Skipping FSDP for memory block {layer_id} (values already wrapped)"
                )
                continue
