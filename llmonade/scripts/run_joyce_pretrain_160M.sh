...
LOGFILE="logs/joyce_pretrain_160M_$(date +%Y%m%d_%H%M%S).log"
echo "Starting Joyce AE pre-training - logging to $LOGFILE"

torchrun --nproc_per_node=1 --nnodes=1 -m llmonade.train \
  --job.config_file llmonade/configs/llmon.toml \
  --job.dump_folder exp/joyce_ae_160M \
  --model.config llmonade/configs/joyce/joyce_pretrain_ae_160m.json \
...
echo "Joyce AE pre-training complete!"
