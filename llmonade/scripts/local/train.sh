...
mkdir -p $path
# NOTE: possibly worth uncommenting for reproducibility - MW
# cp * $path
# cp -r configs $path
# cp -r llmonade   $path
# cp -r 3rdparty/bento/bento $path
...
python -m llmonade.scripts.helpers.convert_dcp_to_hf \
  --path $path \
  --step $steps \
  --config $config \
  --tokenizer $tokenizer
