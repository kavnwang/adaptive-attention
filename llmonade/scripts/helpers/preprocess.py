...
def tokenize(
    ...
    bits_per_token = [
        len(sample.encode(encoding="utf-8")) * 8 / len(input_ids[i])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size", type=int, default=2048, help="Batch size for processing"
    )
    ...
    logger.info(
        f"Loading dataset {args.dataset} {args.dataset_name} {args.dataset_split}"
    )
    ...
    logger.info(
        f"Tokenizing and processing the dataset with batch size {args.batch_size}"
    )
    dataset = dataset.map(
