#!/bin/bash
# autoencoder_160M_predictor_mini.sh
...
mkdir -p exp/autoencoder_predictor_160M_mini
mkdir -p logs
...
# Compression ratio (can be overridden by env)
COMPRESSION_RATIO=${COMPRESSION_RATIO:-0.125}

# Log file with timestamp
LOGFILE="logs/train_autoencoder_160M_predictor_mini_$(date +%Y%m%d_%H%M%S).log"

echo "Starting AE predictor training (r=${COMPRESSION_RATIO}) - logging to $LOGFILE"

torchrun --nproc_per_node=1 --nnodes=1 -m llmonade.train \
  --job.config_file llmonade/configs/llmon.toml \
  --job.dump_folder exp/autoencoder_predictor_160M_mini \
  --model.config llmonade/configs/autoencoder/autoencoder_160m_predictor.json \
  ...
  --training.compression_ratio ${COMPRESSION_RATIO} \
...
echo "AE predictor training complete!"
