...
class AutoencoderForCausalLM(AutoencoderPreTrainedModel, FLAGenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[Any]
        ...
        loss_keep_tokens = self.config.loss_keep_tokens
        ...
        if labels is not None:
            ...
            if loss_keep_tokens > 0:
                keep_len = min(loss_keep_tokens, seq_len_out, labels.size(1))
                hs_for_loss = hidden_states[:, -keep_len:, :]
                labels_for_loss = labels[:, -keep_len:]
            elif masked_tokens_for_loss > 0:
